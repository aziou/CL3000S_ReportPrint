   ...
   /// </summary>
    public sealed class Check
    {
        ...
        public static void Assert(bool assertion, string message)
        {
            ...
            {
                if (!assertion)
                    GlobalUnit.g_MsgControl.OutMessage(message, false);
            }
            else
            {
#if USE_TRACE_ASSERTION
        ...
        public static void Assert(bool assertion)
        {
            if (UseExceptions)
            {
                if (!assertion)
                {
                    GlobalUnit.g_MsgControl.OutMessage(ASSERTION_FAILED,false);
                }
            }
            else
            {
    ...
    /// <summary>
    /// 当后置条件验证失败时触发.一般用于一段操作后对操作结果的验证
    /// </summary>
    [Serializable]
