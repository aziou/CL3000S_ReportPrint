...
{
    public partial class ViewEventCalibrationTime : UserControl
    {
        /// <summary>
        /// 读取校时事件记录数据项目ID
        /// </summary>
        ...
        public ViewEventCalibrationTime()
        {
            Key = ((int)CLDC_Comm.Enum.Cus_EventLogItem.校时记录).ToString().PadLeft(3, '0');
        ...
        private void RefreshGrid()
        {
            ...
            string[] str_Values = new string[] { "事件记录校时次数", "事件记录发生时刻", "事件记录结束时刻" };
            ...
            Data_View.AddSpanHeader(2, 3, strColName);
            strColName = string.Format("【{0}】事件产生（后）信息", strTitleName);
            Data_View.AddSpanHeader(5, 3, strColName);            
        }
        /// <summary>
        /// 刷新表单数据
        /// </summary>
