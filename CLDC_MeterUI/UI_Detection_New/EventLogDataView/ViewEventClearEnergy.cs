...
namespace CLDC_MeterUI.UI_Detection_New.EventLogDataView
{
    ...
    /// 清零事件记录
    /// </summary>
    public partial class ViewEventClearEnergy : UserControl
    {
        /// <summary>
        /// 读取清零事件记录数据项目ID
        /// </summary>
        ...
        public ViewEventClearEnergy()
        {
            Key = ((int)CLDC_Comm.Enum.Cus_EventLogItem.电表清零记录).ToString().PadLeft(3, '0');
        ...
        private void RefreshGrid()
        {
            ...
            string[] str_Values = new string[] { "电表清零总次数", "事件记录发生时刻", "电表清零前正向有功需量" };
            ...
            for (int i = 0; i < MeterGroup.Count; i++)         //循环将电能表数据集合中的时段投切数据值插入到表格中
            {
                ...
                for (int j = 1; j <= 3; j++)
                {
                    ...
                    if (!MeterGroup[i].MeterSjJLgns.ContainsKey(_Key))
                    {
                        ...
                        {
                            for (int k = 0; k < 3; k++)
                                Data_View.Rows[i].Cells[(j - 2) * 3 + k + 1].Value = "";
                        }
                        continue;
                    }
                    ...
                    if (j < 2)
                    {
                        Data_View.Rows[i].Cells[j].Value = MeterGroup[i].MeterSjJLgns[_Key].RecordOther;
                    }
                    else
                    {
                        ...
                        for (int k = 0; k < arrValue.Length; k++)
                            Data_View.Rows[i].Cells[(j - 2) * 3 + k + 1].Value = arrValue[k];
                    }
                }
            }
        }
