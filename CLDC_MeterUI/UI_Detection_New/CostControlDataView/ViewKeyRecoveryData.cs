...
{
    public partial class ViewKeyRecoveryData : UserControl
    {
        /// <summary>
        /// 读取密钥恢复数据项目ID
        /// </summary>
        ...
        public ViewKeyRecoveryData(CLDC_DataCore.Struct.StPlan_CostControl CostControlPlan)
        {
            InitializeComponent();
            if (CostControlPlan.CostControlPrjID != Key)        //如果项目ID不是计度器示值组合误差的ID则退出！！
                return;

            int _ColIndex = Data_View.Columns.Add("Data_Z", "密钥恢复数据内容");
