...
{
    public partial class ViewErrAccord : UserControl
    {
        public ViewErrAccord()
        {
            InitializeComponent();
        }
        ...
        public void SetData(List<CLDC_DataCore.Model.DnbModel.DnbInfo.MeterBasicInfo> MeterGroup,CLDC_DataCore.Struct.StErrAccord CurItem)
        {
            ...
            if(Data_View.Rows.Count != MeterGroup.Count * BwRows)
            {
                ...
                for (int i = 0; i < MeterGroup.Count;i++ )
                {
                    ...
                    {
                        int intCol = i * BwRows + j;
                        string strBwInfo = MeterGroup[i].ToString();
                        ...
                        Data_View[1, intCol].Value = CurItem.lstErrPoint[j].TestPointName;
                    }
                    bColor = !bColor;
                }
            }
                    ...
                    try
                    {
                        for (int j = 0; j < _MeterInfo.MeterErrAccords[_ItemKey].lstTestPoint.Count; j++)
                        {
                            ...
                            if (_MeterInfo.MeterErrAccords[_ItemKey].lstTestPoint.ContainsKey(_subKey))
                            {
                                ...
                                //样品均值
                                Row.Cells[6].Value = _MeterInfo.MeterErrAccords[_ItemKey].lstTestPoint[_subKey].Mea_WcAver;
                                //差值
                                Row.Cells[7].Value = _MeterInfo.MeterErrAccords[_ItemKey].lstTestPoint[_subKey].Mea_Wc;
                                //结论
                                Row.Cells[8].Value = _MeterInfo.MeterErrAccords[_ItemKey].lstTestPoint[_subKey].Mea_ItemResult;
                            }
                        }
                    }
                    catch { }
