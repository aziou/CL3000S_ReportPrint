...
{
    public partial class ViewInfrared : UserControl
    {        
        ...
        public void SetData(List<CLDC_DataCore.Model.DnbModel.DnbInfo.MeterBasicInfo> MeterGroup,string strKey)
        {
            ...
            for (int i = 0; i < MeterGroup.Count; i++)
            {
                if (!MeterGroup[i].YaoJianYn)
                    continue;
                if (!MeterGroup[i].MeterInfraredDatas.ContainsKey(strKey))           //如果多功能集合中不存在改关键字则不处理
                    continue;
                    

                Data_View.Rows[i].Cells[0].Value = MeterGroup[i].MeterInfraredDatas[strKey].Mif_PrjName;
                Data_View.Rows[i].Cells[1].Value = MeterGroup[i].MeterInfraredDatas[strKey].Mif_Prj485Value;
                Data_View.Rows[i].Cells[2].Value = MeterGroup[i].MeterInfraredDatas[strKey].Mif_PrjInfrareValue;
                Data_View.Rows[i].Cells[2].Value = MeterGroup[i].MeterInfraredDatas[strKey].Mif_ItemResult;
            }
