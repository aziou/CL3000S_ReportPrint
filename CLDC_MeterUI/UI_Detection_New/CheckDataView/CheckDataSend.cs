...
{
    public partial class CheckDataSend : UserControl
    {
        ...
        public CheckDataSend(Main parent, CLDC_DataCore.Model.DnbModel.DnbGroupInfo meterGroup, int CheckOrderID, int taiType, int TaiID)
        {
            _csDataFlag.Load();
            InitializeComponent();
        ...
        {
            CLDC_DataCore.Struct.StDataSendForRelay _Item;

            if (MeterGroup.CheckPlan[CheckOrderID] is CLDC_DataCore.Struct.StDataSendForRelay)
            {
                _Item = (CLDC_DataCore.Struct.StDataSendForRelay)MeterGroup.CheckPlan[CheckOrderID];
            }
            else
            {
                return;
            ...
            for (int i = 0; i < MeterGroup.MeterGroup.Count; i++)
            {
                ...
                try
                {
                    ...
                    string _Key = string.Format("{0}{1}{2}", ((int)CLDC_Comm.Enum.Cus_MeterResultPrjID.数据转发试验).ToString(), _csDataFlag.GetDataFlagNo(_Item.ConnProtocolItem).ToString().PadLeft(2, '0'), _Item.PARAMS_LIST);
                    if (MeterGroup.MeterGroup[i].MeterResults.ContainsKey(_Key))            //如果这个项目做过
                    {
                        if (MeterGroup.CheckState == CLDC_Comm.Enum.Cus_CheckStaute.停止检定)        //如果不是在停止状态，则显示进度
                        {
                            Row.Cells[3].Value = MeterGroup.MeterGroup[i].MeterResults[_Key].Mr_chrNote;
                            Row.Cells[4].Value = MeterGroup.MeterGroup[i].MeterResults[_Key].Mr_chrRstValue!=null?MeterGroup.MeterGroup[i].MeterResults[_Key].Mr_chrRstValue:"";
                    ...
                    if (_MeterInfo.Mb_Result == CLDC_DataCore.Const.Variable.CTG_BuHeGe)
                    {
                        ...
                        {
                            cell.ToolTipText = MeterGroup.MeterGroup[i].MeterResults[_Key].AVR_DIS_REASON;
                        }
                    }
                    else
                    {
