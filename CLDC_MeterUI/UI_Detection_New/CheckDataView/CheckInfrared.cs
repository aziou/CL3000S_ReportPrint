...
{
    public partial class CheckInfrared : UserControl
    {
        ...
        public CheckInfrared(Main parent, CLDC_DataCore.Model.DnbModel.DnbGroupInfo meterGroup, int CheckOrderID, int taiType, int TaiID)
        {
            InitializeComponent();
        ...
        private void RefreshGrid(CLDC_DataCore.Model.DnbModel.DnbGroupInfo MeterGroup, int CheckOrderID)
        {
            ...
            if (MeterGroup.CheckPlan[CheckOrderID] is StPlan_Infrared)
            {
            ...
            string _Key = string.Format("{0}{1}{2}", ((int)CLDC_Comm.Enum.Cus_MeterResultPrjID.红外数据比对试验).ToString(), _Item.str_Code,_Item .str_Type );
            for (int i = 0; i < MeterGroup.MeterGroup.Count; i++)
            {
                ...
                if (_MeterInfo.MeterInfraredDatas == null)
                    _MeterInfo.MeterInfraredDatas = new Dictionary<string, CLDC_DataCore.Model.DnbModel.DnbInfo.MeterInfraredData>();
                ...
                if (MeterGroup.MeterGroup[i].MeterInfraredDatas.ContainsKey(_Key))
                {
                    Row.Cells[3].Value = "检定完毕";

                    Row.Cells[4].Value = MeterGroup.MeterGroup[i].MeterInfraredDatas[_Key].Mif_ItemResult;
                }
                else
                {
                ...
                if (Row.Cells[4].Value != null)
                {
                    ...
                    if (Row.Cells[4].Value.ToString() == CLDC_DataCore.Const.Variable.CTG_BuHeGe)      
                    {
                        ...
                        {
                            cell.ToolTipText = MeterGroup.MeterGroup[i].MeterInfraredDatas[_Key].AVR_DIS_REASON;
                        }
                    }
                    else
                    {
            ...
            if (_Control is CLDC_MeterUI.UI_Detection_New.InfraredDataView.ViewInfrared)         
            {
                ((CLDC_MeterUI.UI_Detection_New.InfraredDataView.ViewInfrared)_Control).SetData(MeterGroup.MeterGroup, _Key);
                return;
            }
        ...
        public void RefreshData(CLDC_DataCore.Model.DnbModel.DnbGroupInfo meterGroup, int CheckOrderID)
        {
            ...
            if (!bFind)
            {
                ...
                Tab_Carrier.TabPages.Add("红外数据比对数据");
                CLDC_MeterUI.UI_Detection_New.InfraredDataView.ViewInfrared _Infrared = new CLDC_MeterUI.UI_Detection_New.InfraredDataView.ViewInfrared();
                Tab_Carrier.TabPages[1].Controls.Add(_Infrared);
                _Infrared.Dock = DockStyle.Fill;
                _Infrared.Margin = new System.Windows.Forms.Padding(0);
            }
            this.RefreshGrid(meterGroup, CheckOrderID);
