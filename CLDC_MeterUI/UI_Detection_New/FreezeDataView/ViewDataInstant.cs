...
{
    public partial class ViewDataInstant : UserControl
    {
        private const string Key_One = "00201";                     //瞬时冻结电量在集合中存储的关键字

        public ViewDataInstant()
        {
            InitializeComponent();
        }
        ...
        public void SetData(List<CLDC_DataCore.Model.DnbModel.DnbInfo.MeterBasicInfo> MeterGroup)
        {
            ...
            for (int i = 0; i < MeterGroup.Count; i++)
            {
                ...
                if ((MeterGroup[i].MeterFreezes.ContainsKey(Key_One) && !string.IsNullOrEmpty(MeterGroup[i].MeterFreezes[Key_One].Md_chrValue)))
                {
                    string[] _Values = MeterGroup[i].MeterFreezes[Key_One].Md_chrValue.Split('|');
                    if (_Values.Length != 5) continue;          //如果数据长度不等于3，则表示数据有问题，跳过不处理
        ...
        public void SetData(CLDC_DataCore.Model.DnbModel.DnbInfo.MeterBasicInfo MeterInfo, bool allowedit)
        {
            ...
            if (MeterInfo.MeterFreezes.Count > 0)
            {
                ...
                if ((MeterInfo.MeterFreezes.ContainsKey(Key_One) && !string.IsNullOrEmpty(MeterInfo.MeterFreezes[Key_One].Md_chrValue)))
                {
                    string[] _Values = MeterInfo.MeterFreezes[Key_One].Md_chrValue.Split('|');
                    if (_Values.Length != 5) return ;          //如果数据长度不等于3，则表示数据有问题，跳过不处理
                    for (int j = 0; j < _Values.Length; j++)
                    {
