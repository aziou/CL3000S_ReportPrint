...
{
    public partial class DisplayOver : Base
    {
        /// <summary>
        /// 过载试验项目ID
        /// </summary>
        private const string Key = "4";
        ...
        {
            if (MeterInfo.MeterErrAccords.Count == 0) return;
                ...
                try
                {
                    for (int j = 0; j < MeterInfo.MeterErrAccords[Key].lstTestPoint.Count; j++)
                    {
                        ...
                        if (MeterInfo.MeterErrAccords[Key].lstTestPoint.ContainsKey(_subKey))
                        {
                            ...
                            //填写当前检定点的结论
                            Row.Cells[6].Value = MeterInfo.MeterErrAccords[Key].lstTestPoint[_subKey].Mea_ItemResult;
                        }
                    }
                }
                catch { }
        ...
        public override void SetData(CLDC_DataCore.Model.DnbModel.DnbGroupInfo MeterGroup, bool allowedit)
        {
            ...
            for (int i = 0; i < MeterGroup._Bws; i++)
            {
                ...
                for (int j = 0; j < BwRows; j++)
                {
                    ...
                    bColor = !bColor;
                    Data_View.Rows[RowIndex].Cells[0].Value = MeterGroup.MeterGroup[i].ToString();
                    Data_View.Rows[RowIndex].Cells[1].Value = MeterGroup.MeterGroup[i].MeterErrAccords[Key].lstTestPoint[strSubKey[j]].Mea_PrjName;
                    ...
                    {
                        if (k > 4)
                            Data_View.Rows[RowIndex].Cells[2 + k].Value = "";
                        else
                            Data_View.Rows[RowIndex].Cells[2 + k].Value = Arr_Err[k];
                    }

                    //填写当前检定点的结论
                    Data_View.Rows[RowIndex].Cells[6].Value = MeterGroup.MeterGroup[i].MeterErrAccords[Key].lstTestPoint[_subKey].Mea_ItemResult;
                    #endregion
