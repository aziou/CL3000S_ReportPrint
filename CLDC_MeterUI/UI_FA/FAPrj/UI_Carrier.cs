...
namespace CLDC_MeterUI.UI_FA.FAPrj
{
    ...
    ///
    /// </summary>
    public partial class UI_Carrier : UI_TableBase
    {
        #region--------------私有变量-----------------
        ...
        /// </summary>
        /// <returns></returns>
        public CLDC_DataCore.Model.Plan.Plan_Carrier Copy()
        {
            if (Dgv_Data.Rows.Count == 1) return new CLDC_DataCore.Model.Plan.Plan_Carrier((int)TaiType, "");

            CLDC_DataCore.Model.Plan.Plan_Carrier _Obj = new CLDC_DataCore.Model.Plan.Plan_Carrier((int)TaiType, "");
            ...
            for (int i = 0; i < Dgv_Data.Rows.Count; i++)
            {
                ...
                {
                    if (!this.CheckOK(i)) return new CLDC_DataCore.Model.Plan.Plan_Carrier((int)TaiType, "");


                    bool b_ModuleSwaps = false;

                    b_ModuleSwaps = bool.Parse(((DataGridViewCheckBoxCell)Dgv_Data.Rows[i].Cells[3]).Value.ToString());

                    _Obj.Add(i, Dgv_Data.Rows[i].Cells[1].Value.ToString(),
                               Dgv_Data.Rows[i].Cells[2].Value.ToString(), b_ModuleSwaps
                               );
                }
            }
        ...
        public void LoadPlan(string p_str_PlanName)
        {
            ...
            //【打开一个方案】
            CLDC_DataCore.Model.Plan.Plan_Carrier pcc_CarrierPlan = new CLDC_DataCore.Model.Plan.Plan_Carrier((int)base.TaiType, p_str_PlanName);
