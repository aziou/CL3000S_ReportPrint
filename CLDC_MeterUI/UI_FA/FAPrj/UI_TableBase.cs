...
{
    public partial class UI_TableBase : UserControl
    {
        ...
        {
            if (BaseDataGrid.SelectedRows[0].Index == BaseDataGrid.Rows.Count - 2) return;
            int RowIndex = BaseDataGrid.SelectedRows[0].Index;
        ...
        {
            if (BaseDataGrid.Rows.Count > 2 && RowIndex < BaseDataGrid.Rows.Count - 1)          //如果行数大于2,且选中行不是最后一行
                if (RowIndex == 0)
                {
                    _MoveDown.Enabled = true;
                    _MoveUp.Enabled = false;
                }
                else if (RowIndex == BaseDataGrid.Rows.Count - 2)
                {
                    _MoveDown.Enabled = false;
