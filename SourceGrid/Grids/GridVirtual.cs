...
namespace SourceGrid
{
	...
	{
		private static log4net.ILog m_log = log4net.LogManager.GetLogger(typeof(GridVirtual));
		#region Constructor
		/// <summary>
		/// Grid constructor
		/// </summary>
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			{
				m_log.DebugFormat("Processing OnMouseDown event, because clickPosition was not empty");
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
			...
			m_log.DebugFormat("Processing OnMouseUp event, because clickPosition was not empty");
			if (MouseDownPosition.IsEmpty() == false)
			{
				Cells.ICellVirtual mouseDownCell = GetCell(MouseDownPosition);
				if (mouseDownCell != null)
		...
		private void HandleMouseDoubleClick(Position clickPosition, EventArgs e)
		{
			if (clickPosition.IsEmpty() == true)
				return;
			m_log.DebugFormat("Processing OnMouseDoubleClick event, because clickPosition was not empty");
		...
		private void SetScrollArea()
		{
			...
			//MICK(20)
			int vertPage = GetVisibleRows(false).Count - ActualFixedRows;
			int horPage = GetVisibleColumns(false).Count - ActualFixedColumns;
